const SEGMENT_YMAX = 5;
			...
			{
				segmentGeo = new THREE.BoxBufferGeometry( length, 5, 2 ); 
				var segmentMesh = new THREE.Mesh( segmentGeo, new THREE.MeshLambertMaterial( { color: 0x0000ff } ) );
			...
			{
				segmentGeo = new THREE.BoxBufferGeometry( 5, length, 2 ); 
				var segmentMesh = new THREE.Mesh( segmentGeo, new THREE.MeshLambertMaterial( { color: 0x0000ff } ) );
				segmentMesh.position.x= x;
				segmentMesh.position.y= y;
				segmentMesh.position.z= z;
			...
			function createSlopeXZSegment( x, y, z, length, angleInGrad )
			{
				...
				geoSlope.vertices.push(
					...
					new THREE.Vector3( 0, -SEGMENT_YMAX/2,  SEGMENT_ZMAX/2 ),
					new THREE.Vector3( 0,  SEGMENT_YMAX/2,  SEGMENT_ZMAX/2 ),
					new THREE.Vector3( segmentLength * Math.cos(angle), -SEGMENT_YMAX/2, segmentLength * Math.sin(angle)-SEGMENT_ZMAX/2 ),
					new THREE.Vector3( segmentLength * Math.cos(angle), +SEGMENT_YMAX/2, segmentLength * Math.sin(angle)-SEGMENT_ZMAX/2 ),
					new THREE.Vector3( segmentLength * Math.cos(angle), -SEGMENT_YMAX/2, segmentLength * Math.sin(angle)+SEGMENT_ZMAX/2 ),
					new THREE.Vector3( segmentLength * Math.cos(angle), +SEGMENT_YMAX/2, segmentLength * Math.sin(angle)+SEGMENT_ZMAX/2 )
				);
				...
				var geoSlopeMesh = new THREE.Mesh( geoSlope, new THREE.MeshLambertMaterial( { color: 0x0000ff } ));
			...
			function createSlopeXYSegment( x, y, z, length, angleInGrad )
			{
				...
				geoSlope.vertices.push(
					...
					new THREE.Vector3( 0, -SEGMENT_YMAX/2,  SEGMENT_ZMAX/2 ),
					new THREE.Vector3( 0,  SEGMENT_YMAX/2,  SEGMENT_ZMAX/2 ),
					new THREE.Vector3( segmentLength * Math.cos(angle), segmentLength * Math.sin(angle)-SEGMENT_YMAX/2, -SEGMENT_ZMAX/2 ),
					new THREE.Vector3( segmentLength * Math.cos(angle), segmentLength * Math.sin(angle)-SEGMENT_YMAX/2, +SEGMENT_ZMAX/2 ),
					new THREE.Vector3( segmentLength * Math.cos(angle), segmentLength * Math.sin(angle)+SEGMENT_YMAX/2, -SEGMENT_ZMAX/2 ),
					new THREE.Vector3( segmentLength * Math.cos(angle), segmentLength * Math.sin(angle)+SEGMENT_YMAX/2, +SEGMENT_ZMAX/2 )
				);
				...
				geoSlope.computeBoundingSphere();
				geoSlope.computeVertexNormals();				
				var geoSlopeMesh = new THREE.Mesh( geoSlope, new THREE.MeshLambertMaterial( { color: 0x0000ff, shading: THREE.FlatShading } ));
				geoSlopeMesh.position.x = x;
				geoSlopeMesh.position.y = y;
				geoSlopeMesh.position.z = z;
			...
			function createCurvedSegment( x, y, z, rotX, rotY, rotZ )
			{
				...
				curvedSegment2D.absarc( 0, CURVED_RADIUS, CURVED_RADIUS-SEGMENT_YMAX/2, Math.PI / 2, 0, true );				
				curvedSegment2D.absarc( 0, CURVED_RADIUS, CURVED_RADIUS+SEGMENT_YMAX/2, 0, Math.PI / 2, false );
				
				var extrudeSettings = { depth: 2, bevelEnabled: false };
				var curvedSegmentGeo = new THREE.ExtrudeBufferGeometry( curvedSegment2D, extrudeSettings );				
				
				var meshGeo = new THREE.Mesh( curvedSegmentGeo, new THREE.MeshLambertMaterial( { color: 0x0000ff, side: THREE.DoubleSide } ) );
			...
			function addCurvedSESegment(nextDirection)
			{
				...
				var curvedSegmentSE;
				if (nextDirection === 'N')
					curvedSegmentSE = createCurvedSegment( x-CURVED_RADIUS, y+CURVED_RADIUS, z-SEGMENT_ZMAX/2, 0, 0, 3 * Math.PI / 2 );
				else
					curvedSegmentSE = createCurvedSegment( x-CURVED_RADIUS*2, y, z-SEGMENT_ZMAX/2, 0, 0, 3 * Math.PI / 2 );
			...
			function addCurvedSWSegment(nextDirection)
			{
				...
				var curvedSegment3;
				if (nextDirection === 'E')
					curvedSegment3 = createCurvedSegment( x+CURVED_RADIUS, y+CURVED_RADIUS, z-SEGMENT_ZMAX/2, 0, 0, Math.PI );
				else
					curvedSegment3 = createCurvedSegment( x, y+CURVED_RADIUS*2, z-SEGMENT_ZMAX/2, 0, 0, Math.PI );
			...
			{				
				...
				var curvedSegment ;
				if (nextDirection === 'W')
					curvedSegment = createCurvedSegment(x - CURVED_RADIUS, y-CURVED_RADIUS, z-SEGMENT_ZMAX/2, 0, 0, 0 );
				else
					curvedSegment = createCurvedSegment(x, y - CURVED_RADIUS * 2, z-SEGMENT_ZMAX/2, 0, 0, 0 );
			...
			{				
				...
				newPosition[1] = newPosition[1] + modif * CURVED_RADIUS;
				if (nextDirection === 'S')				
					scene.add(createCurvedSegment( x + CURVED_RADIUS, y + modif * CURVED_RADIUS, z-SEGMENT_ZMAX/2, 0, 0, Math.PI/2 ));
				else 
					scene.add(createCurvedSegment( x + CURVED_RADIUS * 2, y, z-SEGMENT_ZMAX/2, 0, 0, Math.PI/2 ));
			...
			function addLinearSegment(size)
			{
				...
				if (direction === 'E' || direction === 'W')
					...
					addLinearYSegment(size, direction);
			}
			...
			function addTurnToLeft()
			{
				...
				if (startDirection === 'N')
					addCurvedNESegment(nextDirection);
				if (startDirection === 'E')
					addCurvedSESegment(nextDirection);
				if (startDirection === 'S')
					addCurvedSWSegment(nextDirection);
				if (startDirection === 'W')
					addCurvedNWSegment(nextDirection);
			...
			function init() {
				...
				*/
				...
				var curvedSegmentPos = createCurvedSegment( (40 * Math.cos(2 * Math.PI / 360 * angleSlope)) - CURVED_RADIUS, CURVED_RADIUS, 40 * Math.sin(2*Math.PI/360 * angleSlope)+10, 0, 0, 3 * Math.PI / 2  );
			...
			function render() 
			{
				...
				if ( intersects.length > 0 )
				{
					...
					{		
						...
						if (minIntersectedObject == null || minDistance > intersects[i].distance)
						{
